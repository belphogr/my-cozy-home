...
def island_terrain(name, center, rx, ry, edge_z, relief, mat, target, seed, rings=10, segments=64):
    ...
    for ring in range(1,rings+1):
        ...
        for i in range(segments):
            ...
            radius=t*boundary[i]*(1+.035*math.sin(a*3+seed*.4)*(1-t))
    ...
    def add_trunk(x, y, height, radius):
        ...
        for leaf in range(8):
            ...
            for section in range(sections+1):
                t=section/sections
                width=.24*scale*math.sin(math.pi*t)**.7
...
def canopy_cluster(name, center, count, spread, mats, target, seed, scale=1.0, terrain=None, clearings=()):
    ...
    def add_crown(x, y, z, sx, sy, sz, shade):
        base=len(verts);sides=6;rotation=rng.random()*math.tau
        # Three irregular rings make a faceted rounded crown instead of a cone.
        for ring,(height,radius) in enumerate(((-.42,.70),(0,1.0),(.43,.68))):
            for side in range(sides):
                a=rotation+math.tau*side/sides
                wobble=1+rng.uniform(-.09,.09)
                verts.append((x+math.cos(a)*sx*radius*wobble,y+math.sin(a)*sy*radius*wobble,z+height*sz+rng.uniform(-.035,.035)*scale))
        verts.extend([(x,y,z+sz*.64),(x,y,z-sz*.78)])
        top=base+sides*3;bottom=top+1
        for side in range(sides):
            nxt=(side+1)%sides
            lo=base+side;mid=base+sides+side;hi=base+sides*2+side
            lon=base+nxt;midn=base+sides+nxt;hin=base+sides*2+nxt
            faces.extend(((lo,lon,midn,mid),(mid,midn,hin,hi),(hi,hin,top),(lon,lo,bottom)))
    ...
    planted=0
    for _ in range(max(1, int(count*.72))):
        angle = rng.random() * math.tau
        ...
        planted+=1
    obj=mesh_object(name, verts, faces, mats['canopy'], target, indices, (mats['canopy_light'],))
...
def create_district(mats, static, build, growth):
    ...
    for name, center, rx, ry, seed in islands:
        ...
        island_terrain(f'Beach_{name}', center, rx + 2.0, ry + 1.8, -.98, .70, mats['sand'], static, seed)
